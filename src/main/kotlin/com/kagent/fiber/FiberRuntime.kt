...
@Deprecated("Use KAgentRuntime from com.kagent.runtime instead. FiberRuntime is replaced by RuntimeHeart internally.", ReplaceWith("KAgentRuntime", "com.kagent.runtime.KAgentRuntime"))
class FiberRuntime(
    ...
    suspend fun run(block: suspend FiberRuntimeScope.() -> Unit) {
        mutex.withLock {
            ...
            timeline.record(
                com.kagent.fiber.ExecutionRecord(
                    stepId = "runtime",
    ...
    suspend fun <T> derivedSuspend(id: String, compute: suspend () -> T): SuspendableDerivedState<T> {
        val derived = com.kagent.fiber.derivedSuspend(
            id = id,
            tracker = tracker,
            compute = compute
        )
    ...
    suspend fun replayToCheckpoint(name: String) {
        ...
        if (targetCheckpoint != null) {
            val snapshot = com.kagent.memory.snapshot.DefaultMemorySnapshot(
    ...
    suspend fun getStateAtCheckpoint(name: String): MemorySnapshot? {
        ...
        return target?.let {
            com.kagent.memory.snapshot.DefaultMemorySnapshot(
...
@Deprecated("Use KAgentRuntime.from com.kagent.runtime instead. FiberRuntimeScope is internal to the deprecated FiberRuntime.", ReplaceWith("KAgentRuntime", "com.kagent.runtime.KAgentRuntime"))
...
@Deprecated("Use kAgentRuntime from com.kagent.runtime instead.", ReplaceWith("kAgentRuntime(id, memory = memory, block = block)", "com.kagent.runtime.kAgentRuntime"))
fun fiberRuntime(
    id: String = "default",
    memory: Memory,
